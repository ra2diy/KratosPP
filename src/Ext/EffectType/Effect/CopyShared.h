...
// Ignore 黑名单优先 -> Watch 白名单（空=全量）。
// 素材不做"附加器排除"：连锁与否由目标端同名合并闸（Cumulative 等）决定，属配置方责任。

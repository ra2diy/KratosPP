#pragma once
// ============================================================================
// CopyData — Copy 与 AddByExisting 的公共数据基础（同源族）
//
// 两个效果器共用同一套机制：观察指定单位身上正在生效的 AE（Watch/Ignore 名单），
// 把观察结果换成"要附加的内容"（Copy = 命中 AE 本身；AddByExisting = INI 固定名单），
// 按 AttachTo（贴给谁）/ AttachFrom（来源记谁）分发到选定目标，附加一律走现有 Attach 基建。
//
// 本文件定义三组取值枚举、观察组结构与 Copy 的数据类：
//   CopyAttachTo    贴给谁（Copy 支持 Return；AddByExisting 经 CopyParseAttachTo 限三值）
...
// 按首字母解析 AttachTo 取值；allowReturn=false 时不接受 Return（AddByExisting 用）
...
// 观察名单组（主效果器与 AddByExisting 各持一份；过滤判定逻辑共用）
